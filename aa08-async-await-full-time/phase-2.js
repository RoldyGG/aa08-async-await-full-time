...
async function workout(totalTime) {
  ...
  try {
    const timeLeftAfterStretch = await stretch(totalTime);
    const timeLeftAfterRun = await runOnTreadmill(timeLeftAfterStretch);
    const result =  await liftWeights(timeLeftAfterRun);
